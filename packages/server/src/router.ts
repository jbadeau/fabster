...
export const appRouter = t.router({
  ...
  // List skills discovered from installed @fabster plugin packages
  listSkills: t.procedure.query(async () => {
    const { skills } = await loadCatalog();
    return { skills };
  }),

  // List rules discovered from installed @fabster plugin packages
  listRules: t.procedure.query(async () => {
    const { rules } = await loadCatalog();
    return { rules };
  }),

  // List agents discovered from installed @fabster plugin packages
  listAgents: t.procedure.query(async () => {
  ...
  runWorkflow: t.procedure
    ...
    .mutation(async ({ input }) => {
      ...
      const nodeStates: SerializedNode[] = nodes.map((n) => {
        ...
        const base: SerializedNode = {
          ...
          permissions: def.permissions ? {
            ...
            apis: def.permissions.apis ? def.permissions.apis.map((a) => ({ name: a.name, baseUrl: a.baseUrl, scopes: a.scopes ? [...a.scopes] : undefined })) : undefined,
            externalSystems: def.permissions.externalSystems ? def.permissions.externalSystems.map((s) => ({ name: s.name, kind: s.kind, host: s.host })) : undefined,
            secrets: def.permissions.secrets ? [...def.permissions.secrets] : undefined,
          } : undefined,
          gates: [...(def.pre ?? []), ...(def.post ?? [])].map((g) => g.kind),
        };
      ...
      runWorkflow(workflow, { agents, emitter }).catch((err) => {
