...
import { PermissionsBlock, type PermissionsView } from '@/components/catalog-detail';
import {
  ResizablePanelGroup,
  ResizablePanel,
  ResizableHandle,
...
const initialNodes: Node[] = [
  ...
  },
  {
    ...
    data: {
      label: 'Write API Spec', definition: 'todomvc:generate-openapi-spec', type: 'task', order: 5, reasoning: 'medium', agent: 'Tank',
  ...
  },
  {
    ...
    data: {
      label: 'Implement Backend', definition: 'todomvc:implement-backend', type: 'task', order: 9, reasoning: 'high', agent: 'Tank',
  ...
  },
  {
    ...
    data: {
      label: 'Implement Frontend', definition: 'todomvc:implement-frontend', type: 'task', order: 10, reasoning: 'high', agent: 'Tank',
...
function ComposeCanvas({ runId }: { runId?: string }) {
  ...
  // Canvas column: when logs are open it's a vertically-resizable split
  // (drag the handle to resize the log panel); otherwise the canvas fills and
  // the collapsed log header sits pinned at the bottom.
  const canvasColumn =
    isExecutionMode && logsOpen ? (
      <ResizablePanelGroup orientation="vertical" className="h-full">
        <ResizablePanel id="canvas" defaultSize="68%" minSize="25%">
          {canvasArea}
        </ResizablePanel>
        <ResizableHandle withHandle />
        <ResizablePanel id="logs" defaultSize="32%" minSize="12%">
      ...
      <div className="flex h-full flex-col">
        <div className="flex-1 min-h-0">{canvasArea}</div>
        {isExecutionMode && (
          <div className="shrink-0 border-t bg-card">{logsHeaderButton}</div>
        )}
      </div>
    );
  ...
  return (
    <>
      {/* Horizontally-resizable split: canvas column | properties sidebar. */}
      <ResizablePanelGroup
      ...
      >
        ...
        {selectedNode && <ResizableHandle withHandle />}
        {selectedNode && (
          <ResizablePanel id="props" defaultSize="28%" minSize="18%" maxSize="55%">
            <PropertiesPanel
              node={selectedNode}
...
function PropertiesPanel({
  ...
  return (
    <div className="h-full border-l overflow-y-auto bg-card">
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {/* Permissions: tools, filesystem, network, APIs, external systems, secrets */}
        {data.permissions && (
          <>
            <Separator />
            <div className="flex flex-col gap-1.5">
          ...
          <div className="flex flex-col gap-4 py-4">
            ...
            {type === 'task' && (
              <div className="flex flex-col gap-2">
                ...
                <Select value={reasoning} onValueChange={(v) => setReasoning(v ?? 'medium')}>
